...
# ...AND 125 MORE (Karan's call): with the west column 2725 as built, the
# service door up the arch above it runs from the column's top to where this
# wall lands on the arch, and at 600 that was 584 wide.  The whole front
# stands 725 north of the builder's line now, one straight run duct cheek to
# apse — no step — and the door is 776, the same as the east one.
KIT_BUMP = 725                       # how far north the wall goes
...
# on it — 26 degrees, 776 clear with the front at 725 (Karan's call: the
# whole wall moved the least that makes the door usable, no step).  It
# swings into the gallery like the east one, and the north counter's east
# end is cleared floor in front of it.
...
# higher: from the column's top at 8400 to the kitchen wall at 7800, 776
# clear, with 30 degrees of arch below it backed solid by the column.
...
GALLERY = (GAL_CX, GAL_CY, GAL_R, T_GAL,
           [(_A1 + 0.2, _A0 - 0.2),              # below the two springings
            ...
            (_ACW, _BN0K),                       # 776 service door, to the kitchen — above the 2725 column
